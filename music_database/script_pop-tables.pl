...
my $script_name = "script_pop-tables.pl";
...
# The information about every song is available from the third level of <dict> of
# the XML file: whatever is contained between the <dict> </dict> labels is the
# info about the song.   But there is no only information about songs, it also
# contain information about podcasts, movies, etc, things that I am not interested
# in.  Therefore, those won't be caught
            ...
            if ($name) {
                if ($band) {
                    if ($album) {
                        ...
                        # and I am taking from the "grouping" field if available
...
say "---- Some Stats --------------";
say "Printed:     " . $c_total;
say "  - Songs with known Artist:    " . $c_bands;
say "  - Songs with known Albums:    " . $c_albums;
say "  - Songs                  :     " . $c_songs;
...
# OUTPUT FILE
my $outfile = "djmusic_pop_all_tables.sql";
...
script_pop-tables.pl - Extract music info from iTunes XML file and generates a
SQL file to pop all the info into the djmusic postgreSQL database

=head1 SYNOPSIS

perl script_pop-tables.pl -f=itunes2014grouping_proc.xml

=head1 OUTPUT

djmusic_pop_all_tables.sql
